...
async function main() {
  ...
  const currentDirectory = await app.vault.getAbstractFileByPath(
...
async function modifyFile(file) {
  ...
  const Expression = RegExp(/(?<=banner: )\"\!\[\[Banner\/descriptionbanner\.png\]\]\"/gm);
  const Text = await app.vault.read(file);

  const match =await  Text.match(Expression);

  let replaceText = ``;

  if (match) {
    replaceText = `"[[80-Gallery/Banner/Description.png]]"`;
    console.log(
      `match for expression \n`,
      Expression,
      `\nreplacing with ${replaceText}`
    );
  }
