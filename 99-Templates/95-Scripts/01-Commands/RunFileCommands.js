...
async function RunFileCommands(File) {
  ...
  for (const Line of CommandLines) {
    ...
    if (BreakScript) {
      new Notice(
        "You provided null/invalid value for a mandatory input.\nScript cannot continue."
      );
      break;
    }
  }
...
const Commands = {
  ...
  BuildDocument: async (Argument) => {
    // Argument = await tp.user.ReplaceValues(Argument)
    console.log(
      `01-Commands: RunFileCommands: Commands.BuildDocument(): Building Document\n`
    );
  ...
  BuildSubDocument: async (Argument) => {
    ...
    console.log(
      `01-Commands: RunFileCommands: Commands.BuildSubDocument():\nBuilding Sub Document`
    );
  ...
  BuildSubNote: async (Argument) => {
    ...
    Meta[parentKey] = `\"[[${filePath}|${fileName}]]\"`;
    console.log(
      `01-Commands: RunFileCommands: Commands.BuildSubNote():\nBuilding Sub Note`
    );
    console.log(Meta[parentKey]);
