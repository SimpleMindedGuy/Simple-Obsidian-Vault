 ...
 * 
 * @typedef {string|null} OverviewKey - Stores the Name of variable/key that will be used to specify which key is used to link the Ovewview File, with the documents associated with that oveview file.
 ...
 * - will be used to write formatted daate using the {@link currentDate} value
...
async function RemoveCommandBlock(file) {
  ...
  return await app.vault.modify(
    ...
    .catch(() => {
      console.warn(`failed failed to remove command block`)
      return false
    })
  // Text = await 
}
...
async function Menu(Argument, tp) {
  ...
  if (!Argument) {
    ...
    list = []
    console.warn(`Setting menu to an empty menu`)
    console.log(menu)
    return list
  }
 ...
 * - if the provided Date value is valid according the to the format, then the {@link currentDate} will be set to the date value, in an ISO format.
 * - if there is not date value provided, then the {@link currentDate} is set to a new date value in an ISO format.
 * 
 * 
 * 
 * @date 16/12/2023
...
/**
 * Replaces expressions that hold the a keyname in the provided Text {@link Argument}
...
/**
 * Finds a directory with the provided name {@link Name}, at the path/directory  {@link Path}
 * 
 ...
 * @param {Number} padding - Amount of digites in the number string.
